#!/usr/bin/env python3
"""Fix Script: Data/MC comparison plots (Issue #1).

Changes from original 03_data_mc_comparisons.py:
  - Data as raw counts (events/bin), MC scaled to match data integral
  - Lower panel: (Data - MC) / sigma_Data  (pull-style)
  - Remove "Axis 0" text from ratio panels (mplhep bug)

Session: Wolfgang (fix agent) | Phase 2
"""
...
DATA_DIR = Path("/n/holystore01/LABS/iaifi_lab/Lab/sambt/LEP/ALEPHMC").parent / "ALEPH"
...
def suppress_axis0_text(*axes):
    """Remove spurious 'Axis 0' text sometimes added by mplhep."""
    for ax in axes:
        for txt in ax.texts:
            if "Axis" in txt.get_text():
                txt.set_visible(False)
        # Also check the figure-level texts
        for txt in ax.figure.texts:
            if "Axis" in txt.get_text():
                txt.set_visible(False)
...
def make_ratio_plot(
    ...
    """Data/MC comparison: data as raw counts, MC scaled to data integral.
    Lower panel: (Data - MC) / sigma_Data."""
    ...
    # Data as errorbar (raw counts)
    centers = 0.5 * (bins[:-1] + bins[1:])
    data_counts = h_data.values()
    data_err = np.sqrt(data_counts)
    ax.errorbar(centers, data_counts, yerr=data_err,
                fmt="ko", markersize=4, label=data_label, zorder=5)

    # Pull panel: (Data - MC) / sigma_Data
    mc_scaled_vals = h_mc_scaled.values()
    pull = np.zeros_like(data_counts)
    mask = data_err > 0
    pull[mask] = (data_counts[mask] - mc_scaled_vals[mask]) / data_err[mask]
    ...
    # Suppress "Axis 0" text right before saving (mplhep adds it late)
    suppress_axis0_text(ax, rax)
    # Also hide the x-axis label on the upper panel (sharex puts it there)
    ax.set_xlabel("")
...
def main():
    log.info("=" * 60)
    log.info("Fix Script: Data/MC Comparisons (Issue #1)")
    ...
    make_ratio_plot(data_pmag, mc_pmag, np.linspace(0.2, 10, 50),
                    r"$p$ [GeV/$c$]", "hugo_pmag_data_mc")
    make_ratio_plot(data_pmag, mc_pmag, np.logspace(np.log10(0.2), np.log10(50), 50),
                    r"$p$ [GeV/$c$]", "hugo_pmag_log_data_mc", logy=True)
    make_ratio_plot(data_theta, mc_theta, np.linspace(0.3, 2.85, 40),
                    r"$\theta$ [rad]", "hugo_theta_data_mc")
    make_ratio_plot(data_phi, mc_phi, np.linspace(-np.pi, np.pi, 40),
                    r"$\phi$ [rad]", "hugo_phi_data_mc")
    make_ratio_plot(data_pt, mc_pt, np.linspace(0.2, 8, 50),
                    r"$p_T$ [GeV/$c$]", "hugo_pt_data_mc")

    data_thrust = np.asarray(data_sel["Thrust"])
    mc_thrust = np.asarray(mc_sel["Thrust"])
    make_ratio_plot(data_thrust, mc_thrust, np.linspace(0.6, 1.0, 50),
    ...
    make_ratio_plot(data_nch, mc_nch, np.arange(0, 50, 1),
    ...
    make_ratio_plot(data_hemi, mc_hemi, np.arange(0, 25, 1),
    ...
    make_ratio_plot(data_thrust_ch, mc_thrust_ch, np.linspace(0.6, 1.0, 50),
                    r"Thrust$_{\mathrm{charged}}$", "hugo_thrust_charged_data_mc")

    # Copy key figures to analysis_note/figures
    import shutil
    for name in ["hugo_pmag_data_mc", "hugo_theta_data_mc", "hugo_phi_data_mc",
                 "hugo_nch_data_mc", "hugo_thrust_data_mc", "hugo_hemi_mult_data_mc",
                 "hugo_pmag_log_data_mc", "hugo_pt_data_mc", "hugo_thrust_charged_data_mc"]:
        for ext in [".pdf", ".png"]:
            src = FIG_DIR / f"{name}{ext}"
            dst = AN_FIG_DIR / f"{name}{ext}"
            if src.exists():
                shutil.copy2(src, dst)

    log.info("\nAll data/MC comparison plots regenerated with pull panels.")
