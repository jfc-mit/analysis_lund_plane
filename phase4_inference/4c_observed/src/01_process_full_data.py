#!/usr/bin/env python3
"""Phase 4c Script 01: Process ALL 6 data files (full statistics).

Applies the same event selection and Lund plane construction as Phase 3
and Phase 4b, but with NO subsampling. Processes all 3.05M events.
Saves per-file histograms for per-year stability checks.
...
M_PI = 0.13957  # Pion mass in GeV (PDG 2024)
...
def process_data_file_full(filepath):
    """Process a single data file with NO subsampling (100% of events)."""
    log.info("DATA (FULL): %s", filepath.name)
    t0 = time.time()
    ...
    # NO subsampling -- process all events
    ...
    dt = time.time() - t0
    log.info(
        "  %s: %d total -> %d selected -> %d hemi-cut "
        "-> %d hemi, %d splits (%.1fs)",
        filepath.name, n_total, n_after_sel,
...
def main():
    log.info("=" * 70)
    log.info("Phase 4c Script 01: Process ALL Real Data (100%%)")
    log.info("Session: Emeric | Full unblinded data")
    log.info("=" * 70)
    ...
    # Process data files in parallel (6 files, 6 workers)
    n_workers = 6
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(process_data_file_full, f): f for f in data_files}
    ...
    log.info(
        "FULL Data: %d hemispheres, %d splittings (%.3f splits/hemi)",
        data_n_hemi, data_n_split, data_n_split / max(data_n_hemi, 1),
    )

    # Save combined full result
    np.savez(
        OUT_DIR / "data_full_lund_emeric.npz",
        h2d=data_h2d,
    ...
    per_file_hemi_arr = np.array([per_file_n_hemi[fn] for fn in file_names])
    np.savez(
        OUT_DIR / "data_full_per_file_emeric.npz",
        h2d_per_file=per_file_arrays,
        n_hemi_per_file=per_file_hemi_arr,
    ...
    with open(OUT_DIR / "cutflow_full_emeric.json", "w") as fp:
        json.dump(cutflow_summary, fp, indent=2)

    log.info("\nAll results saved to %s", OUT_DIR)

    # Crosscheck against Phase 3 full data histogram
    d3 = np.load(
        str(Path("/n/home07/anovak/work/slopspec/analyses/lund_jet_plane")
            / "phase3_selection" / "outputs" / "data_lund_ingrid.npz")
    )
    h2d_phase3 = d3["h2d"]
    n_hemi_phase3 = int(d3["n_hemispheres"])
    log.info("\n=== Cross-check vs Phase 3 full data ===")
    log.info("Phase 3 hemispheres: %d, Phase 4c hemispheres: %d", n_hemi_phase3, data_n_hemi)
    log.info("Phase 3 h2d sum: %.0f, Phase 4c h2d sum: %.0f", h2d_phase3.sum(), data_h2d.sum())
    diff = np.abs(data_h2d - h2d_phase3)
    log.info("Max bin difference: %.1f", diff.max())
    if diff.max() < 1:
        log.info("PASS: Full data histograms match Phase 3 exactly.")
    else:
        log.warning("WARN: Discrepancy with Phase 3 -- max bin diff = %.1f", diff.max())

    # Print summary table
    log.info("\n=== FULL DATA CUTFLOW ===")
    log.info(
        "%-50s %10s %10s %10s %10s %10s",
        "File", "Total", "Selected", "Hemi-cut", "Hemispheres", "Splittings",
    )
    log.info("-" * 100)
    for cf in sorted(data_cutflows, key=lambda c: c["file"]):
        log.info(
            "%-50s %10d %10d %10d %10d %10d",
            cf["file"], cf["total"], cf["after_selection"],
            cf["after_hemi_cut"], cf["hemispheres"], cf["splittings"],
        )
    log.info("-" * 100)
    log.info(
        "%-50s %10d %10d %10d %10d %10d",
        ...
        cutflow_summary["total_splittings"],
    )
