...
%
% Doc 4b v1 — 10% data results added to expected results
% Compile with: tectonic ANALYSIS_NOTE_doc4b_v1.tex
%
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/hugo_pmag_data_mc.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/hugo_theta_data_mc.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/hugo_phi_data_mc.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/hugo_pt_data_mc.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/hugo_thrust_data_mc.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/hugo_thrust_charged_data_mc.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/hugo_nch_data_mc.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/hugo_hemi_mult_data_mc.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/hugo_bin_population_reco.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/hugo_migration_fraction.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/ingrid_correction_factor_map.pdf}
\caption{Bin-by-bin correction factor map $C(i,j) = N_\text{genBefore}/N_\text{reco}$
  across the Lund plane.  The correction is close to unity in the
  densely populated core and increases at the kinematic boundaries
  where detector effects are larger.  The minimum value of 1.17
  reflects the $\sim\!79\%$ event selection efficiency.  This map is
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/ingrid_efficiency_map.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/ingrid_diagonal_fraction_map.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/felix_split_closure_kt.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/felix_split_closure_dtheta.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/nikolai_closure_pulls.pdf}
\caption{Pull distribution for the split-sample closure test using
  the combined uncertainty (Eq.~\ref{eq:combined-sigma}).
  The pull mean is $-0.14$ and the pull standard deviation is $0.83$,
  consistent with a unit Gaussian (expected: mean $= 0$, std $= 1$).
  The slightly narrow distribution (std $< 1$) indicates the combined
  uncertainty is mildly conservative, which is acceptable.  For 58 bins,
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/felix_bbb_vs_ibu_kt.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/felix_bbb_vs_ibu_dtheta.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/felix_response_matrix.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/felix_diagonal_fraction.pdf}
...
charged-particle reconstruction efficiency between data and MC.
The per-track efficiency of the ALEPH TPC is $\sim\!98.5\%$ for
isolated tracks with $p > 200$~MeV/$c$ in the barrel
region~\cite{ALEPH:1995aqm}.

\textbf{Method:} Randomly drop 1\% of charged tracks from the MC
reco-level sample, recompute the Lund plane, and rederive correction
factors.  The 1\% drop rate is chosen as approximately 1$\sigma$ above
the measured $\sim\!1.5\%$ per-track inefficiency, providing a
conservative systematic envelope.

\textbf{Impact:} Maximum relative shift of 5.7\% at boundary bins;
mean relative shift $\sim\!1\%$ across populated bins.  The impact
is largest in the low-$k_T$ region where soft tracks dominate and
track reconstruction is least efficient.

\textbf{Interpretation:} This is a subdominant systematic, consistent
with the 0.5--1.5\% range found in published ALEPH
measurements~\cite{ALEPH:1998vtv}.
...
\textbf{Method:} Smear track momenta by $\pm 10\%$ of the nominal
resolution.  Recompute the Lund plane and rederive corrections.

\textbf{Impact:} Maximum relative shift of 1.1\%; mean $\sim\!0.3\%$.
This is negligible compared to other sources, reflecting the excellent
ALEPH tracking resolution.

\textbf{Interpretation:} Deeply subdominant.  The momentum resolution
is far better than the bin widths, so smearing has minimal effect on
bin migration.
...
\textbf{Method:} Smear track angles by $\pm 1$~mrad.

\textbf{Impact:} Maximum relative shift of 100\% at one boundary bin
with very low population; mean $\sim\!5\%$.  The large boundary-bin
shift reflects the sensitivity of the opening angle $\Delta\theta$
near the edge of the phase space.  In the core of the Lund plane,
the impact is $<\!2\%$.

\textbf{Interpretation:} The boundary sensitivity is driven by a single
bin and does not affect the physics content of the measurement.  This
systematic is subdominant in the populated region.
...
\textbf{Method:} Vary the track momentum threshold from 150 to
250~MeV/$c$ (nominal: 200~MeV/$c$).

\textbf{Impact:} Maximum relative shift of 107\% (low threshold) and
86\% (high threshold) at boundary bins.  Mean impact $\sim\!10\%$.
This is one of the larger systematics because the low-$k_T$ region
of the Lund plane is populated by soft tracks near the threshold.

\textbf{Interpretation:} Dominant in the non-perturbative region.
The standard ALEPH threshold of 200~MeV/$c$ is well-established and
the $\pm 50$~MeV/$c$ variation covers the range of thresholds used
in published ALEPH analyses.
...
\textbf{Method:} Vary the transverse impact parameter cut from 1.5 to
2.5~cm (nominal: 2.0~cm).

\textbf{Impact:} Maximum relative shift of 10\% (tight cut) and
0.01\% (loose cut); mean $\sim\!1\%$.  The asymmetry indicates
that tightening the $d_0$ cut preferentially removes secondary
decay products (e.g., from $K^0_S$, $\Lambda$), which contribute
to the wide-angle, low-$k_T$ region.

\textbf{Interpretation:} Subdominant.  The impact is confined to
...
\textbf{Method:} Vary the thrust cut from 0.6 to 0.8 (nominal: 0.7).

\textbf{Impact:} Maximum relative shift of 6.5\% (looser cut) and
1.1\% (tighter cut); mean $\sim\!1\%$.  Loosening the cut includes
more multi-jet events where the hemisphere assignment is less clean.

\textbf{Interpretation:} Subdominant.  The thrust cut primarily
affects the event sample composition, not the per-hemisphere splitting
pattern.
...
\textbf{Method:} Vary the minimum charged multiplicity from 4 to 6
(nominal: 5).

\textbf{Impact:} Maximum relative shift of 0.01\%; mean $\sim\!0.01\%$.
This is negligible because the minimum-track cut has essentially no
effect on the selected sample (it was pre-applied during archiving).

\textbf{Interpretation:} Negligible.

\subsection{$E_\text{ch}$ minimum}
\label{sec:syst-ech}

The minimum charged energy cut removes low-energy events from
non-hadronic sources.

\textbf{Method:} Compute the charged energy sum from track momenta
and vary the threshold from 12 to 18~GeV (nominal $\sim$15~GeV from
\texttt{passesAll}).

\textbf{Impact:} Maximum relative shift of 0.5\%; mean $\sim\!0.1\%$.

\textbf{Interpretation:} Negligible.  The charged energy cut has
high acceptance for genuine hadronic $Z$ decays.
...
momentum measurement uncertainties.

\textbf{Method:} Smear track momenta within their resolution and
recompute the thrust axis.  Measure the hemisphere migration rate
(fraction of particles that change hemisphere assignment).

\textbf{Impact:} Maximum relative shift of 0.02\%; mean $\sim\!0.01\%$.
The thrust axis is determined by the full event momentum flow and
is extremely stable against per-track momentum smearing.

\textbf{Interpretation:} Negligible.  The thrust axis resolution
is sub-milliradian.

\subsection{MC model dependence}
\label{sec:syst-model}

The MC model dependence is the dominant systematic, accounting for
the sensitivity of the correction factors to the assumed truth-level
distribution.

\textbf{Method:} Reweight the PYTHIA~6.1 gen-level Lund plane by a
2D tilt of $\pm 20\%$, rederive correction factors from the reweighted
MC, and apply to the nominal reco.  The reweighting simulates the
effect of using a different MC generator (e.g., PYTHIA~8 or
HERWIG~7) with a different truth-level shape.

\textbf{Impact:} Maximum relative shift $\sim\!10\%$; largest in the
non-perturbative region ($\ln k_T < 0$) where hadronization models
differ most.  In the perturbative core ($\ln k_T > 1$), the impact
is $<\!3\%$.

\textbf{Interpretation:} This is the dominant systematic, consistent
with findings from ATLAS~\cite{ATLAS:2020bbn} (5--30\% model
dependence) and CMS~\cite{CMS:2024mlf}.  The reweighting approach
provides a lower bound on the true model dependence; a full alternative
generator comparison with PYTHIA~8 and HERWIG~7 at particle level would
provide a more complete assessment.  \tbd{Full generator comparison
to be performed in Phase~4c.}
...
\textbf{Method:} Compare correction factors derived from a 10-file MC
subset vs.\ a 30-file MC subset.

\textbf{Impact:} Variable across bins; driven by the limited MC
statistics in the subset.

\textbf{Interpretation:} This replaces the previously committed
``unfolding method'' systematic (IBU-vs-BBB difference), which was
removed because it quantified IBU's known failure, not a genuine
...
collinear region of the Lund plane~\cite{LHCb:2025viq}.

\textbf{Method:} Reweight the MC $b$-quark fraction by
$\pm 2\%$ and rederive correction factors.

\textbf{Impact:} Maximum relative shift of 0.2\%.

\textbf{Interpretation:} Negligible.  The $b$-fraction is known to
sub-percent precision and the reweighting has a small effect.

\subsection{ISR modelling}
\label{sec:syst-isr}

Initial-state radiation at the $Z$ pole is a small effect
(beamstrahlung energy loss $\sim\!0.1\%$ of $\sqrt{s}$).

\textbf{Method:} Compare the thrust axis computed from all gen-level
charged particles with and without \texttt{pwflag} $= -11$ particles
(ISR/material interactions).

\textbf{Impact:} Maximum relative shift of 0.1\%.
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/nikolai_syst_breakdown.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/nikolai_syst_impact_kt.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/nikolai_syst_impact_dtheta.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/oscar_per_year_kt.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/oscar_per_year_dtheta.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/nikolai_correlation_matrix.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/felix_lund_plane_corrected.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/ingrid_lund_kt_data_mc.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/ingrid_lund_dtheta_data_mc.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/hugo_lund_kt_projection.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/hugo_lund_dtheta_projection.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/nikolai_uncertainty_summary.pdf}
\caption{Total, statistical, and systematic uncertainty as a function
  of the populated bin index.  The statistical uncertainty (orange)
  is subdominant in the core of the Lund plane and dominant at the
  kinematic boundaries.  The systematic uncertainty (blue) dominates
  in the bulk, driven primarily by MC model dependence.  The total
  uncertainty (black) ranges from $\sim\!1\%$ in the most populated
  bins to $\sim\!8\%$ at the boundaries.}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/hugo_reco_gen_1d_comparison.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/hugo_correction_factor_preview.pdf}
\caption{Left: reco/gen 1D comparison from Phase~2 exploration,
  showing the detector-level distortion of the Lund plane projections.
  Right: correction factor preview ($N_\text{gen}/N_\text{reco}$, post-selection
  only), ranging from 0.8 to 1.2 in the core.  These demonstrate that
  the detector response is close to unity and the bin-by-bin correction
  is well-motivated.}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/oscar_lund_plane_10pct_corrected.pdf}
\caption{Corrected primary Lund jet plane density
  $\rho(\ln(1/\Delta\theta), \ln(k_T/\text{GeV}))$ at charged-particle
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/oscar_ratio_10pct_vs_expected.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/oscar_pull_map_10pct.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/oscar_data_mc_reco_ratio_2d.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/oscar_cutflow_comparison.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/ingrid_closure_kt.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/ingrid_closure_dtheta.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/ingrid_closure_pulls.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/felix_stress_ln_kt.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/felix_stress_ln_1_over_dtheta.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/felix_stress_2d_correlated.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/felix_closure_pulls.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/felix_correlation_matrix.pdf}
\caption{Left: closure pull distribution from the Phase~4a initial
  computation (prior to split-sample re-analysis).  Right: correlation
  matrix from the initial computation.  Both were superseded by the
  updated analysis (Figures~\ref{fig:closure-pulls}
  and~\ref{fig:correlation-matrix}), which includes bootstrap with
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/felix_syst_impact_kt.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/felix_syst_impact_dtheta.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.38\linewidth]{figures/felix_syst_breakdown.pdf}\hspace{1em}%
\includegraphics[height=0.38\linewidth]{figures/felix_uncertainty_summary.pdf}
