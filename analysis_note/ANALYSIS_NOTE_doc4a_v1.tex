...
%
% Doc 4a v1 — Expected results from MC pseudo-data
% Compile with: tectonic ANALYSIS_NOTE_doc4a_v1.tex
%
...
populated.  This note presents expected results based on MC
pseudo-data.  The total uncertainty ranges from $\sim\!1$--$8\%$
(relative) in the densely populated core of the plane, dominated
by MC model dependence in the non-perturbative (low-$k_T$) region
and by statistical precision at the kinematic boundaries.  The split-sample
closure test passes with $\chi^2/\text{ndf} = 40.71/58$ ($p = 0.96$)
when the combined uncertainty (Poisson plus correction-factor noise) is
used.  All 12 split-sample stress tests pass.  Iterative Bayesian
...
reconstruction and generator levels.  This constitutes the first full
...
to be performed in Phase~4b/4c.}
...
\tbd{Full per-year Lund plane comparison to be performed on real
data in Phase~4b.}
...
\tbd{Comparison to PYTHIA~8 Monash and HERWIG~7 particle-level
  predictions will be performed in Phase~4b/4c.}
...
This analysis note presents the expected results for the measurement
of the primary Lund jet plane density in hadronic $Z$ decays at
$\sqrt{s} = 91.2$~GeV using archived ALEPH data.  The measurement
uses approximately 3.05 million events (5.69 million thrust
hemispheres), corrected to charged-particle level using bin-by-bin
correction factors derived from PYTHIA~6.1 Monte Carlo.
...
\item \textbf{Single MC generator.}  Only PYTHIA~6.1 is available with
  ...
  most.  \emph{Mitigation:} Phase~4b/4c comparison with standalone
  PYTHIA~8 and HERWIG~7 at particle level.
...
\item \textbf{Hardness variable [D13].}  Energy ordering as a
  ...
  ordering choice is unquantified.  \emph{Mitigation:} Phase~4b/4c
  implementation.
