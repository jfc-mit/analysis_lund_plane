...
%
% Doc 4c v1 — FINAL: full observed data results (session: Philippa)
% Compile with: tectonic ANALYSIS_NOTE_doc4c_v1.tex
%
...
Figure~\ref{fig:lund-corrected-expected} shows the corrected
two-dimensional primary Lund jet plane density obtained by applying the
bin-by-bin correction factors (Section~\ref{sec:bbb}) to the MC
pseudo-data.  This expected result validates the correction chain
(algebraic identity for same-MC closure) and establishes the reference
against which data is compared.
...
The full-data corrected Lund plane density
(Section~\ref{sec:results-full}) is consistent with the PYTHIA~6.1
expected result within systematic uncertainties (diagonal
...
% =====================================================================
% 8b. RESULTS FROM FULL DATA (PRIMARY RESULT)
% =====================================================================
\subsection{Results from full data}
\label{sec:results-full}

The full ALEPH dataset is processed using the identical analysis chain
validated on the 10\% subsample (Section~\ref{sec:results-10pct}).
This constitutes the primary result of the analysis.

\subsubsection{Full data processing summary}

All six data files covering the 1992--1995 data-taking period are
processed without subsampling.
...
The per-file cutflow confirms uniform selection efficiencies across all
data-taking periods:

\begin{table}[htbp]
\centering
\caption{Per-file cutflow for the full dataset.  All periods have
  selection efficiencies within 93.2--93.5\%, confirming time-independent
  detector performance.}
\label{tbl:full-perfile}
\begin{tabular}{lccccc}
\toprule
Period & Raw & Selected & Hemi-cut & Hemispheres & Efficiency \\
\midrule
1992 & 551\,474 & 518\,770 & 514\,820 & 1\,029\,640 & 93.4\% \\
1993 & 538\,601 & 506\,313 & 502\,452 & 1\,004\,904 & 93.2\% \\
1994 P1 & 433\,947 & 408\,028 & 404\,768 & 809\,536 & 93.2\% \\
1994 P2 & 447\,844 & 421\,031 & 417\,796 & 835\,592 & 93.4\% \\
1994 P3 & 483\,649 & 454\,563 & 451\,048 & 902\,096 & 93.5\% \\
1995 & 595\,095 & 559\,679 & 555\,310 & 1\,110\,620 & 93.3\% \\
\midrule
\textbf{Total} & \textbf{3\,050\,610} & \textbf{2\,868\,384}
  & \textbf{2\,846\,194} & \textbf{5\,692\,388} & \textbf{93.3\%} \\
\bottomrule
\end{tabular}
\end{table}

As a cross-check, the full-data histogram was compared bin-by-bin against
the Phase~3 data histogram.  The maximum bin difference is $< 1$ count,
confirming that the Phase~4c processing reproduces Phase~3 exactly
(same code, same data).

\subsubsection{Correction procedure}

The bin-by-bin correction factors from the full MC (40~files, Phase~3)
are applied to the full data.  The correction factors
...
\subsubsection{Normalization verification}
...
\subsubsection{Corrected full-data Lund plane (the primary result)}
...
per-bin density values annotated.
...
\caption{Corrected Lund plane density from full data with per-bin
  numerical values annotated.  The 58 populated bins span four orders
...
\subsubsection{Comparison to MC expected result}
\label{sec:full-vs-expected}

The full-data corrected density is compared to the MC expected result
(PYTHIA~6.1 genBefore truth) bin-by-bin.  The $\chi^2$ is computed using
...
\subsubsection{1D projections with uncertainty bands}
...
\subsubsection{Reco-level data/MC comparison (full data)}

\begin{table}[htbp]
\centering
\caption{Reco-level data/MC Lund plane density ratio from the full
  dataset.  The mean ratio of 1.005 confirms excellent detector
  simulation calibration.}
\label{tbl:full-reco-ratio}
\begin{tabular}{lc}
\toprule
Metric & Value \\
\midrule
Mean ratio (data/MC reco) & 1.005 \\
...
\subsubsection{Cutflow comparison (full data)}

\begin{table}[htbp]
\centering
\caption{Selection efficiency comparison between full data and MC at
  reco level.  Data and MC efficiencies agree to better than 0.2\%.}
\label{tbl:full-cutflow}
\begin{tabular}{lcccc}
\toprule
Stage & Data eff.\ & MC eff.\ & Ratio \\
\midrule
Input & 1.0000 & 1.0000 & 1.000 \\
After selection & 0.9403 & 0.9417 & 0.999 \\
After hemi cut & 0.9330 & 0.9347 & 0.998 \\
\bottomrule
\end{tabular}
\end{table}
...
full statistical power.  No evidence for time-dependent detector effects.
...
subsample.  The $\chi^2/\text{ndf} = 0.73$ ($p = 0.94$) confirms that
the 10\% subsample was statistically representative.  The density ratio
is $1.007 \pm 0.036$, consistent with unity.  The pull distribution has
mean $+0.08$ and std $0.85$, both consistent with expectations for a
10\% subsample vs.\ the full dataset.
